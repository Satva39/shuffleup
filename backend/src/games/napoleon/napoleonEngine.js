...
function assertPlayerCount(players) {
  if (players.length !== PLAYER_COUNT) {
    throw new Error(`Napoleon requires exactly ${PLAYER_COUNT} players.`);
  }
}
...
function getActiveBidderCount(game) {
  return game.players.filter((player) => !player.passed).length;
}
...
function submitBid(game, playerId, bidInput) {
  ...
  if (!isValidBid(bid, game.highestBid)) {
    ...
    throw new Error(`Bid must be between ${MIN_BID} and ${MAX_BID}.`);
  }
...
function choosePartnerCard(game, playerId, card) {
  ...
  const inBlind = game.blind.some((item) => item.id === game.calledCard.id);
  ...
  if (!inOwnHand && !inBlind) {
    ...
    const exists =
      game.players.some((player) =>
        player.cards.some((item) => item.id === game.calledCard.id),
      ) || inBlind;
...
function takeBlindAndDiscard(game, playerId, discardIds) {
  if (game.phase !== "blind" || game.napoleonId !== playerId) {
  ...
  const combined = [...napoleon.cards, ...game.blind];

  const selected = discardIds.map((cardId) =>
  ...
  napoleon.cards = combined.filter((card) => !discardSet.has(card.id));
...
function resolveTrick(game) {
  ...
  const allPlayed = game.players.every((player) => player.cards.length === 0);
...
function finishRound(game) {
  ...
  const partner = game.partnerId ? getPlayer(game, game.partnerId) : null;

  const teamPoints = napoleon.roundPoints + (partner ? partner.roundPoints : 0);

  const teamResult = calculateTeamResult(teamPoints, game.contract.amount);
  ...
  game.roundResult = {
    ...
    success: teamResult.success,
    deltas,
    discardedScoringCards: game.discardedCards.filter((card) => card.public),
  };
...
function markPlayerConnection(game, playerId, connected, socketId = null) {
  const player = getPlayer(game, playerId);
  if (!player) return;
...
function getWinner(game) {
  return [...game.players].sort((a, b) => b.score - a.score)[0] || null;
}
...
function publicPlayer(player, game, viewerId) {
  ...
  const isNapoleon = player.id === game.napoleonId;
...
function getPublicState(game, viewerId) {
  ...
  const publicState = {
    ...
    players: game.players.map((player) => publicPlayer(player, game, viewerId)),
    ...
    lastCompletedTrick: game.lastCompletedTrick
      ? {
          ...
          cards: game.lastCompletedTrick.cards.map((play) => ({
            playerId: play.playerId,
            card: publicCard(play.card),
          })),
        }
    ...
    completedTrickCount: game.completedTricks.length,

    calledCard:
      viewer.id === game.napoleonId && game.calledCard
        ? { ...game.calledCard }
        : game.partnerRevealed && game.calledCard
          ? { ...game.calledCard }
          : null,

    napoleonId: game.napoleonId,

    partnerId: game.partnerRevealed ? game.partnerId : null,

    partnerRevealed: game.partnerRevealed,
    alone: game.alone,

    discardedScoringCards: game.discardedCards
      .filter((card) => card.public)
      .map(publicCard),

    yourCards: viewer.cards.map(publicCard),

    yourBlind: viewer.id === game.napoleonId ? game.blind.map(publicCard) : [],

    yourBid: viewer.bid ? { ...viewer.bid } : null,

    roundResult: game.roundResult || null,

    winner: game.status === "game-complete" ? getWinner(game) : null,
