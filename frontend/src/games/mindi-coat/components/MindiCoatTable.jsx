...
export default function MindiCoatTable({
    ...
    const gameFinished = state.status === "game-complete" || state.phase === "game-complete";
    const winningTeam = state.winnerTeam;
    const winnerLabel = winningTeam === "A" ? "Team A" : winningTeam === "B" ? "Team B" : "Winner";
    ...
    return (
        <main className="mindi-page">
            <div className="mindi-shell">
                ...
                <ScoreBoard scores={state.scores} targetScore={state.targetScore} tens={tens} />
                    ...
                    <div className="mindi-table">
                        ...
                        <div className="mindi-center">
                            <div className="mindi-center-tools">
                                <TrumpDisplay suit={state.trumpSuit} revealed={state.trumpRevealed} />
                                <TurnIndicator
                            ...
                            {state.canOpenHukum && (
                                <div className="mindi-hukum-action">
                                    <div>
                                        ...
                                        <span>
                                            Open Hukum to reveal the hidden card. The player who hid it gets
                                            the card back and may use it normally.
                                        </span>
                                    </div>
                                    <button className="mindi-primary" type="button" onClick={openHukum}>
                                        Open Hukum
                                    </button>
                                </div>
                            )}
                ...
                <section className="mindi-action-area">
                    {state.phase === "trump-select" && state.trumpSelectorId === user.id && (
                        <TrumpSelector
                            hand={state.hand}
                    ...
                    {state.phase === "trump-select" && state.trumpSelectorId !== user.id && (
                        <div className="mindi-waiting">
                            {seatPlayers[state.currentSeat]?.username || "The selector"} is choosing the hidden Hukum.
                        </div>
                    )}
                    ...
                    {state.trumpRevealed && state.trumpCard && (
                        <div className="mindi-hukum-reveal">
                            <strong>
                                Hukum opened: {state.trumpCard.rank}{state.trumpCard.suit}
                            </strong>
                            <span>
                                {seatPlayers[state.hukumOpenedById]?.username || "A player"} opened it. {" "}
                                {seatPlayers[state.trumpSelectorId]?.username || "The Hukum holder"} now has
                                the card and may use it normally.
                            </span>
                        </div>
                    )}
                    ...
                    {!gameFinished && <GameResult winnerTeam={state.winnerTeam} scores={state.scores} />}
                    {state.canStartNextHand && (
                        <button className="mindi-primary next-hand" type="button" onClick={nextHand}>
                            Start Next Hand
                        </button>
                    )}
                </section>
