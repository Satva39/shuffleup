...
export default function PlayingCard({ card, disabled = false, selected = false, hidden = false, onClick }) {
    if (hidden) {
        return <span className="mindi-card-back" aria-label="Hidden card" />;
    }
    if (!card) return null;
    return (
        <button
            type="button"
            className={`${cardClass(card)} ${selected ? "selected" : ""}`}
            disabled={disabled}
