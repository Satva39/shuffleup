...
export default function SattePeSattaTable({
  state,
  user,
  error,
  connected,
  ...
  const meTurn =
    state.currentPlayerId === user.id && state.status === "playing";
  const legalMoves = state.me?.legalMoves || [];
  const legalCards = (state.me?.hand || []).filter((card) =>
    legalMoves.includes(card.id),
  );
        ...
        <div className="sps-main-grid">
          <section className="sps-table-card">
            <div className="sps-table-topline">
              ...
              <GameAnimation
                as="div"
                key={`turn-${state.turnNumber}`}
                variant="round"
                className="sps-round-meta"
              >
            ...
            <div className="sps-action-bar">
              <button
                className="sps-pass-button"
                type="button"
                onClick={pass}
                disabled={!meTurn || legalMoves.length > 0}
              >
                PASS
              </button>
              <span>
