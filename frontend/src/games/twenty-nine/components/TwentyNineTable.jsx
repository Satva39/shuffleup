...
export default function TwentyNineTable({ state, error, connected, submitBid, selectTrump, playCard, nextHand, user }) {
    const players = state.players || [];
    const bySeat = Object.fromEntries(players.map((player) => [player.seat, player]));
    const me = players.find((player) => player.id === user?.id);
    const canChooseTrump = state.phase === "trump-selection" && state.canSelectTrump;
            ...
            <div className="twenty-nine-shell">
                <header className="twenty-nine-topbar">
                    ...
                    <div className="twenty-nine-top-meta">
                        ...
                        <span className={`twenty-nine-connection ${connected ? "online" : "offline"}`}>{connected ? "Connected" : "Reconnecting…"}</span>
                    </div>
                </header>
                ...
                <section className="twenty-nine-table-wrap">
                    <div className="twenty-nine-table">
                        ...
                        <PlayerSeat player={bySeat.N} position="north" isCurrent={state.currentSeat === "N"} isSelf={bySeat.N?.id === user?.id} />
                        <PlayerSeat player={bySeat.W} position="west" isCurrent={state.currentSeat === "W"} isSelf={bySeat.W?.id === user?.id} />
                        <PlayerSeat player={bySeat.E} position="east" isCurrent={state.currentSeat === "E"} isSelf={bySeat.E?.id === user?.id} />
                        ...
                        <section className="twenty-nine-center">
                            ...
                            <TrickArea trick={state.trick} players={players} />
                            {completed && state.trick.length === 0 && (
                                <div className="twenty-nine-last-winner">Last trick: {completed.winnerSeat} won</div>
                            )}
                        </section>
                ...
                <section className="twenty-nine-controls">
                    {state.phase === "bidding" && (
                        <BiddingPanel state={state} onBid={submitBid} onPass={() => submitBid(null)} />
                    )}
                    {canChooseTrump && (
                        <section className="twenty-nine-panel trump-picker">
                            ...
                            <div className="twenty-nine-trump-options">
                                {trumpOptions.map((option) => (
                                    <button key={option.suit} type="button" onClick={() => selectTrump(option.suit)}>
